...
function valid_form(parent_bl,name,email,err_b){
    ...
    jQuery.ajax({
        url:"/mistake/make_order/",
        type:"post",
        data:"course_id=6&name=" + $('.name_lin').val() + "&phone=" + $('#phone_num').val() + "&email=" + $('.email_lin').val(),
        ...
        success:function (msg) {
            ...
            if (msg == 2) {
					...
					$('#h1_form').hide();
					$('#table_form').hide();
					$('.lind_f').html('Спасибо! Теперь подтвердите свое желание получить видео, нажав на специальную ссылку в письме');
            }else{
