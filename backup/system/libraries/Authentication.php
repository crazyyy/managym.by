...
class Authentication {
	...
	{
		return $this->CI->session->userdata('profile_id') ? TRUE : FALSE;
	}
	 ...
	 * @return void
	 */
	function sign_in($profile_id, $remember = FALSE)
	{
		$remember ? $this->CI->session->cookie_monster(TRUE) : $this->CI->session->cookie_monster(FALSE);

		$this->CI->session->set_userdata('profile_id', $profile_id);

		$this->CI->load->model('profile/profile_model');

		$this->CI->profile_model->update_last_signed_in_datetime($profile_id);
	...
	function sign_out()
	{
		$this->CI->session->unset_userdata('profile_id');
	}
	...
	{
		$this->CI->load->helper('profile/phpass');
...
/* Location: ./application/modules/profile/libraries/Authentication.php */